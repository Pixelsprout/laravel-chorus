...
// Storage key
const getLatestHarmonicIdKey = (userId?: string | number) => `chorus_latest_harmonic_id_${userId || 'guest'}`;
...
export class ChorusCore {
    ...
    /**
     * Setup the ChorusCore with a userId
     */
    public setup(userId: number | null): void {
        this.userId = userId;
    ...
    private getLatestHarmonicId(): string | null {
        return localStorage.getItem(getLatestHarmonicIdKey(this.userId));
    }
    ...
    private saveLatestHarmonicId(id: string): void {
        localStorage.setItem(getLatestHarmonicIdKey(this.userId), id);
    }
