...
{
    public function getInitialData(Request $request, string $table): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            if (!isset($modelMap[$table])) {
                return response()->json(['error' => 'Invalid table name', 'table' => $table], 404);
            }
            ...
            if (!method_exists($instance, 'getSyncFields')) {
            ...
            if ($initialSync) {
                ...
                return response()->json([
                    'latest_harmonic_id' => $latestHarmonic ? $latestHarmonic->id : null,
                    'records' => $records
                ]);
            }
            ...
            $harmonics = $harmonicsQuery->orderBy('id')
                ->take($limit)
                ->get();
            
            // Format the response with the latest harmonic ID
            $latestHarmonicId = $harmonics->count() > 0 
                ? $harmonics->last()->id 
