...
Browser-based database that stores synchronized data locally for offline access and instant queries.
</Accordion>
...
### 1. Laravel Model Events (Default)
```php
...
### 2. Database Triggers (Future Enhancement)
```sql
-- More reliable for bulk operations and external changes
CREATE TRIGGER messages_insert_trigger
AFTER INSERT ON messages
FOR EACH ROW
INSERT INTO harmonics (table_name, record_id, operation, data)
VALUES ('messages', NEW.id, 'create', JSON_OBJECT('id', NEW.id, ...));
```
