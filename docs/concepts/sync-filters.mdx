---
title: "Sync Filters"
description: "Control which records are synchronized to each user with powerful filtering mechanisms."
---
...
The most common filter restricts records to those owned by the current user:
...
Filter based on complex relationships:
...
class Comment extends Model
{
    ...
    protected $syncFields = [
        'id', 'content', 'post_id', 'user_id', 'is_approved', 'created_at'
    ];
    ...
    protected function syncFilter(): Builder
    {
        ...
        return $this->where(function($query) use ($user) {
            ...
            $query->where('user_id', $user->id)
                  // Include approved comments on posts user can access
                  ->orWhere(function($q) use ($user) {
                      $q->where('is_approved', true)
                        ->whereHas('post', function($postQuery) use ($user) {
                            // User can access public posts or posts they authored
...
Adjust filters based on the current context:
...
class Task extends Model
{
    ...
    protected $syncFields = [
        'id', 'title', 'status', 'assignee_id', 'project_id', 'due_date'
    ];
    ...
    protected function syncFilter(): Builder
    {
        ...
        // Check if user is viewing a specific project
        $currentProject = session('current_project_id');
        
        if ($currentProject) {
            // Filter to specific project with user access check
            return $query->where('project_id', $currentProject)
                         ->whereHas('project.members', function($q) use ($user) {
                             $q->where('user_id', $user->id);
                         });
        }
        
        // Default: tasks assigned to user or created by user
        return $query->where(function($q) use ($user) {
...
Never use raw user input in filters:
...
class Article extends Model
{
    ...
    protected function syncFilter(): Builder
    {
        ...
        // return $this->whereRaw("department_id = {$departmentId}");
    }
}
```
...
{
    public function test_sync_endpoint_respects_filters()
    {
        $user = User::factory()->create();
